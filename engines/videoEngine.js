...
async function buildVideo(videoPaths, voicePath, musicPath, outputPath) {
  ...
  // Step 1: Concat all pre-trimmed clips (already correct duration each)
  const lines = videoPaths.map(v => `file '${v}'`).join('
');
  fs.writeFileSync(concatList, lines);

  // Concat + add vignette + dark overlay
  ...
  // Step 2: Mix audio - voice 100% + music 20% with fade
  if (musicPath && fs.existsSync(musicPath)) {
    const fade = 3;
    const musicFilter = `[1:a]volume=0.2,afade=t=in:st=0:d=${fade},afade=t=out:st=${Math.max(0, voiceDuration - fade)}:d=${fade}[music];[0:a]volume=1.0[voice];[voice][music]amix=inputs=2:duration=first[outa]`;
    execSync(`ffmpeg -y -i "${voicePath}" -i "${musicPath}" -filter_complex "${musicFilter}" -map "[outa]" -c:a aac "${tmpAudio}"`, { stdio: 'inherit' });
  } else {
  ...
  // Cleanup
  [concatList, silentVideo, tmpAudio].forEach(f => { try { fs.unlinkSync(f); } catch(e) {} });
